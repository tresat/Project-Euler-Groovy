...
class Problem11SolverMark1 extends AbstractProblemSolver implements Problem11Solver {
  @Override
  public Integer getProblemNumber() {
    return 11
  }

  def List diagonalize(List n) {
    def result = []

    def lim = n.size()-4
    for (i in 0..lim) {
      for (j in 0..lim) {
        result << [n[i][j], n[i+1][j+1], n[i+2][j+2], n[i+3][j+3]]
      }
    }
  ...
  def maxProduct(List n) {
    def productMap = n.groupBy {
      group -> group.inject(1) {
        val, acc -> acc *= val
      }
    }
  ...
  BigInteger solve() {
    ...
    for (i in 0..<nums.size()) {
      rotatedNums << nums.collect {
        row -> row[i]
      }
    }

    def verticalGroups = rotatedNums.collect {
      row -> row.collate(4, 1, false)
    }.flatten().collate(4)

    def diagGroups1 = diagonalize(nums)

    def reversedNums = nums.collect {
      it.reverse()
    }
